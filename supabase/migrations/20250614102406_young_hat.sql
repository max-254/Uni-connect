/*
  ...
  1. New Tables
    ...
    - `security_logs` - Track security-related events
  2. Security
    - Enable RLS on all tables
    - Add policies for proper access control
    - Add two-factor authentication support to profiles
  3. Changes
    - Create helper function for user identification
...
CREATE POLICY IF NOT EXISTS "Only admins can modify permissions"
  ON public.permissions
  FOR ALL
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Anyone can view role permissions"
  ON public.role_permissions
  FOR SELECT
  USING (true);

CREATE POLICY IF NOT EXISTS "Only admins can modify role permissions"
  ON public.role_permissions
  FOR ALL
  USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can view their own security logs"
  ON public.security_logs
  FOR SELECT
  USING (user_id = current_user_id());

CREATE POLICY IF NOT EXISTS "Admins can view all security logs"
  ON public.security_logs
  FOR SELECT
  USING (
    EXISTS (
