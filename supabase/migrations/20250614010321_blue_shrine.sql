/*
  ...
  1. New Tables
    ...
    - `user_sessions` - Manages user login sessions
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'update_audit_log_user_info_trigger'
  ) THEN
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'applications') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'applications_audit_trigger') THEN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'universities') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'universities_audit_trigger') THEN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'student_profiles') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'student_profiles_audit_trigger') THEN
  ...
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'parsed_documents') THEN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'parsed_documents_audit_trigger') THEN
